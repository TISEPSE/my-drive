    ...
    <aside className="w-60 flex-shrink-0 bg-white dark:bg-background-dark border-r border-slate-200 dark:border-border-dark flex flex-col z-20">
      ...
      <div className="h-14 flex items-center px-5 border-b border-slate-200 dark:border-border-dark">
        <div className="flex items-center gap-2.5 text-primary">
          ...
          <h1 className="text-slate-900 dark:text-white text-[13px] font-bold tracking-tight">CloudSpace</h1>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2.5 flex flex-col gap-0.5">
        ...
        <div className="mb-4 px-0.5">
          <button
            ...
            className="w-full flex items-center justify-center gap-1 bg-primary hover:bg-blue-600 text-white font-semibold py-2 px-3 rounded-lg shadow-lg shadow-blue-500/20 transition-all text-sm"
          >
        ...
        <div className="flex flex-col gap-1">
          <p className="px-2.5 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 mt-1">Stockage</p>
          ...
          <div>
            ...
            >
              ...
              >
                <span className={`material-symbols-outlined ${isDriveActive ? 'fill-current' : ''}`}>
                  folder
                </span>
                Mon Drive
              </div>
            ...
            return (
              ...
              >
                {item.icon === 'github' ? GITHUB_SVG : (
                  <span className={`material-symbols-outlined ${isActive ? 'fill-current' : ''} ${item.path === '/starred' && isActive ? 'text-amber-500' : ''}`}>
                    {item.icon}
                  </span>
                )}
                {item.label}
      ...
      <div className="p-3.5 border-t border-slate-200 dark:border-border-dark">
        ...
        <p className="text-[10px] text-slate-500 dark:text-slate-500">{storage.formatted_used} sur {storage.formatted_limit} utilisés</p>
